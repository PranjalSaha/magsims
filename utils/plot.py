...
df["Time"] = pd.to_datetime(
    df["Time"],
    format="%d %b %Y %H:%M:%S.%f",  # DD mmm YYYY HH:MM:SS.fff
    errors="raise"
)
...
ax1.set_title("Auxiliary Magnetic Field Components Over Time",
              fontsize=TITLE_FONTSIZE)
ax1.legend(fontsize=LEGEND_FONTSIZE)
ax1.grid(GRID_ENABLED, alpha=GRID_ALPHA)

# Save the figure
with open(f"{RESULTS_DIR}aux_mag_body.pkl", "wb") as f:
...
# Save the figure
with open(f"{RESULTS_DIR}ang_vel_inrt.pkl", "wb") as f:
...
# Save the figure
with open(f"{RESULTS_DIR}ang_vel_body.pkl", "wb") as f:
...
with open(f"{RESULTS_DIR}hysteresis.pkl", "wb") as f:
...
with open(f"{RESULTS_DIR}torque_inrt.pkl", "wb") as f:
...
with open(f"{RESULTS_DIR}torque_body.pkl", "wb") as f:
...
with open(f"{RESULTS_DIR}mag_mmt_body.pkl", "wb") as f:
...
with open(f"{RESULTS_DIR}mag_mmt_inrt.pkl", "wb") as f:
